...
class CInode : public MDSCacheObject, public InodeStoreBase, public Counter<CInode> {
 public:
  ...
  void maybe_export_ephemeral_random_pin();
  void maybe_export_ephemeral_distributed_pin(bool update=false);
  void set_export_pin(mds_rank_t rank);
  void set_export_ephemeral_random_pin(double prob=0.0);
  ...
  bool is_distributed_ephemerally_pinned() const {
    return state_test(STATE_DISTEPHEMERALPIN);
  }
  bool is_random_ephemerally_pinned() const {
    return state_test(STATE_DISTEPHEMERALPIN);
  }
